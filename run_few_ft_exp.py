...
def run_accuracy(model, tokenizer, model_name, dataset_name, k, num_eval_items = 5000, max_new_tokens=6):
    ...
    run = wandb.init(project="few-ft-accuracy", reinit=True)
    wandb.alert(title=f"few-ft-Acc-{dataset_name}", text=f"model_name_{model_name}-dataset_name_{dataset_name}")
    ...
    results_path = Path(f"results/few-sample-ft/accuracy_results/{model_name}/{dataset_name}/{k}-shot/acc-results.json")
    generations_path = Path(f"results/few-sample-ft/accuracy_results/{model_name}/{dataset_name}/{k}-shot/generations.json")
...
def run_activations(model, tokenizer, model_name, dataset_name, k, num_eval_items = 5000):
    ...
    run = wandb.init(project="few-sampleft-activations", reinit=True)
    wandb.alert(title=f"few-sample-ft-Act-{dataset_name}", text=f"model_name_{model_name}-dataset_name_{dataset_name}")
    ...
    for batch_start_idx in tqdm(range(0, len(dataset), batch_size)):
        ...
        try:
            ...
            for x in range(len(batch_hidden_states)):
                ...
                for layer_idx in range(curr_hidden_state.shape[0]):
                    save_path = Path(f"results/few-sample-ft/activations/{model_name}/{dataset_name}/{k}-shot/layer-{layer_idx}/layer_{layer_idx}_index-{true_index}.safetensors")
        ...
        indices_path = Path(f"results/few-sample-ft/logits/{model_name}/{dataset_name}/{k}-shot/logit_data.json")
...
def main():
    ...
    training_set_ids = config["training_set_ids"]
    num_training_samples_list = config["num_training_samples_list"]


    for training_set_id in training_set_ids:
            ...
            for model_name in models:
                for dataset_name in datasets:
                    ...
                    if lora_adapter_name == 'final':
                        lora_adapter_path = Path(f"finetune-outputs/few-sample/{model_name}/{dataset_name}/{num_training_samples}-samples/{training_set_id}/final")
